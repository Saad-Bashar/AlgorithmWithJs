...
test('anagrams function exists', () => {
  expect(typeof anagram1).toEqual('function');
});

test('"hello" is an anagram of "llohe"', () => {
  expect(anagram1('hello', 'llohe')).toBeTruthy();
});

test('"Whoa! Hi!" is an anagram of "Hi! Whoa!"', () => {
  expect(anagram1('Whoa! Hi!', 'Hi! Whoa!')).toBeTruthy();
});

test('"One One" is not an anagram of "Two two two"', () => {
  expect(anagram1('One One', 'Two two two')).toBeFalsy();
});

test('"One one" is not an anagram of "One one c"', () => {
  expect(anagram1('One one', 'One one c')).toBeFalsy();
});

test('"A tree, a life, a bench" is not an anagram of "A tree, a fence, a yard"', () => {
  expect(
    anagram1('A tree, a life, a bench', 'A tree, a fence, a yard')
  ).toBeFalsy();
});